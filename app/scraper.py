...
HISTORICO_PRECOS = {
    "SP": {
        ...
        2024: [252, 248, 245, 250, 258, 265, 272, 278, 268, 260, 255, 262],
        2025: [268, 275, 282, 290, 298, 305, 310, 308, 298, 292, 288, 295],
    },
    "MT": {
        ...
        2024: [248, 244, 241, 246, 254, 261, 268, 274, 264, 256, 251, 258],
        2025: [264, 271, 278, 286, 294, 301, 306, 304, 294, 288, 284, 291],
    },
    "GO": {
        ...
        2024: [250, 246, 243, 248, 256, 263, 270, 276, 266, 258, 253, 260],
        2025: [266, 273, 280, 288, 296, 303, 308, 306, 296, 290, 286, 293],
    },
}
...
# ---------------------------------------------------------------------------
# SCRAPING — CEPEA preço atual
# ---------------------------------------------------------------------------
def scrape_cepea_atual():
    """Tenta buscar o preço atual do boi gordo SP no CEPEA."""
    cache_key = "cepea_atual"
    ...
    try:
        ...
        # CEPEA exibe o preço numa tabela com id "imagenet-indicador1"
        tabela = soup.find("table", {"id": "imagenet-indicador1"})
        if tabela:
            linhas = tabela.find_all("tr")
            for linha in linhas[1:3]:  # primeira linha de dados
                cols = linha.find_all("td")
                if len(cols) >= 2:
                    preco_str = cols[1].text.strip().replace(".", "").replace(",", ".")
                    preco = float(preco_str)
                    resultado = {"preco": preco, "fonte": "CEPEA/Esalq", "atualizado": datetime.now().isoformat()}
                    cache_preco[cache_key] = resultado
                    return resultado
    except Exception as e:
        logger.warning(f"CEPEA scraping falhou: {e}")

    # Fallback: última média conhecida do histórico
    ano_atual = datetime.now().year
    mes_atual = datetime.now().month
    historico_sp = HISTORICO_PRECOS["SP"]
    ano_ref = min(ano_atual, 2025)
    preco_fallback = historico_sp.get(ano_ref, historico_sp[2025])[mes_atual - 1]
    resultado = {
        "preco": preco_fallback,
        "fonte": "Histórico CEPEA (fallback)",
...
def comparativo_chuva_seca(estado: str):
    ...
    for ano, precos in sorted(dados.items()):
        precos_chuva = [p for p, c in zip(precos, chuva_mask) if c]
        precos_seca = [p for p, c in zip(precos, chuva_mask) if not c]
        resultado.append({
            "ano": ano,
